...
class BeamImplTest(tft_unit.TransformTestCase):
    ...
    def testWithTimeStamps(self):
        ...
        output_timestep_1 = [values[0]]
        for k in self.create_transform_output(timestamp_1):
            output_timestep_1.append(k)

        output_timestep_2 = [values[1]]
        for k in self.create_transform_output(timestamp_2):
            output_timestep_2.append(k)
